...
const APPLET_REGISTRY = [
  ...
  { id: "schededit", name: "SchedEdit", desc: "Weekly class planner with shared account sync and reminders.", apk: "maru-schededit.apk", icon: "applet-schededit.svg" },
];
...
function renderSettingsPanel() {
  ...
  const elevationStatusHtml = !elevationToken
    ...
    : `<span class="link-settings-value" style="color:rgba(122,255,160,0.9)">Unlocked</span>`;
  const elevationBodyHtml = elevationExpanded
    ? elevationToken
      ? `
        <div class="link-settings-note">
          Elevation is unlocked for this device.
        </div>
      ...
      `
      : `
        <div class="link-settings-note">Unlock this once if you want special helper features on this device.</div>
      ...
      `
    : elevationToken
      ? `<div class="link-settings-note">Elevation is unlocked for this helper.</div>`
