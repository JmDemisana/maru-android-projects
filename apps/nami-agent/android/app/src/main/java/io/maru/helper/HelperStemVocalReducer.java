...
final class HelperStemVocalReducer {
    ...
    static boolean hasBundledModel(Context context) {
        try (InputStream ignored = context.getAssets().open(MODEL_ASSET_PATH)) {
            return true;
    ...
    private ByteBuffer loadModelFile() throws IOException {
        // Prefer bundled asset (built into the APK)
        ByteBuffer bundled = tryLoadBundledFd();
        if (bundled != null) return bundled;
        ByteBuffer bundledStream = tryLoadBundledStream();
        if (bundledStream != null) return bundledStream;

        // Fallback: load from internal storage (downloaded by user)
        ...
        throw new IOException("Stem model not found in assets or internal storage.");
    }

    private ByteBuffer tryLoadBundledFd() {
        try {
    ...
    private ByteBuffer tryLoadBundledStream() {
