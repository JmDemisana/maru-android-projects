...
public class MarucastFragment extends Fragment {
    ...
    public View onCreateView(
        ...
        typePinButton = view.findViewById(R.id.btn_type_receiver_pin);
        ...
        typePinButton.setOnClickListener(v -> showTypePinDialog());
    ...
    private void refreshStatus() {
        ...
        activity.maybePublishLinkedMarucastReceiverRoute(status);
        ...
        styleActionButton(typePinButton, !receiverToken.isEmpty(), true, receiverToken.isEmpty());
    ...
    private void showTypePinDialog() {
        AlertDialog.Builder builder = new AlertDialog.Builder(requireContext());
        builder.setTitle("Type 6-digit PIN");
        ...
        builder.setPositiveButton("Connect", (dialog, which) -> {
            ...
            if (pin.length() != 6) {
                getMainActivity().setMarucastPanelError("Enter a 6-digit PIN.");
                refreshStatus();
                return;
            }

            getMainActivity().setMarucastPanelMessage("Looking up PIN...");
            refreshStatus();

            MainActivity activity = getMainActivity();
            String serverOrigin = activity.getLinkedMarucastServerOrigin();
            if (serverOrigin == null || serverOrigin.isEmpty()) {
                serverOrigin = io.maru.helper.HelperStorage.resolvePublicSiteOrigin(requireContext());
            }

            String finalServerOrigin = serverOrigin;
            new Thread(() -> {
                JSONObject response = activity.lookupMarucastPin(pin, finalServerOrigin);
                boolean success = response.optBoolean("success", false);
                ...
                activity.runOnUiThread(() -> {
                    ...
                    if (success && !token.isEmpty()) {
                        activity.applyLinkedReceiverFromRaw("maruhelper://marucast?receiverToken=" + token + "&siteOrigin=" + Uri.encode(finalServerOrigin), "Receiver linked via PIN!");
                    } else {
                        String msg = error.isEmpty() ? "Could not find a receiver with that PIN." : error;
                        activity.setMarucastPanelError(msg);
                    }
                    refreshStatus();
                });
            }, "MarucastPinLookup").start();
        });
