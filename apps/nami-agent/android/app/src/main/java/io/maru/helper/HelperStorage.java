...
public final class HelperStorage {
    ...
    private static final String KEY_ACCOUNT_TOKEN = "helper-account-token";
    ...
    public static String getAccountToken(Context context) {
        String stored = getPrefs(context).getString(KEY_ACCOUNT_TOKEN, "");
        return stored == null ? "" : stored.trim();
    }

    public static void persistAccountToken(Context context, String rawToken) {
        String token = rawToken == null ? "" : rawToken.trim();
        if (token.isEmpty()) {
            getPrefs(context).edit().remove(KEY_ACCOUNT_TOKEN).apply();
            return;
        }

        getPrefs(context).edit().putString(KEY_ACCOUNT_TOKEN, token).apply();
    }
