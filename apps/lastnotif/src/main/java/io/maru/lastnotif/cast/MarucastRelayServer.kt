...
class MarucastRelayServer(private val port: Int = 48543) {
    ...
    private fun handleStatusRequest(socket: Socket, out: OutputStream) {
        ...
        val statusMap = mapOf(
            ...
            "mediaDurationMs" to CastMediaState.durationMs,
            "mediaPlaying" to CastMediaState.isPlaying,
            "mediaPlaybackSpeed" to (if (CastMediaState.isPlaying) 1.0 else 0.0),
            "mediaPositionCapturedAtMs" to System.currentTimeMillis(),
            "mediaPositionMs" to (CastMediaState.getEstimatedPosition() + MarucastForegroundService.lyricsDelayOffsetMs),
            "mediaTitle" to (CastMediaState.title ?: "Unknown Track"),
            "serviceName" to "maru-audio-suite",
            "sampleRate" to 44100,
            "vocalProcessingKind" to (if (MarucastForegroundService.isKaraokeMode) "karaoke" else "none"),
    ...
    private fun handleArtworkRequest(out: OutputStream) {
        try {
            val bitmap = CastMediaState.artworkBitmap
            if (bitmap != null) {
